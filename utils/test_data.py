...
class ChainTestData:
    ...
    def genera_dati_deliveroo():
        ...
        fake = Faker("it_IT")
        TENANT_DEMO = "Ristorante Demo (demo_tenant_test)"
    ...
    @staticmethod
    def genera_dati_nuova_categoria_costo():
        ...
        return {
            ...
            "codice": f"CATTEST_{suffisso}",
            "nome": f"Categoria Test {suffisso}",
            "iva":random.randint(5, 120),
            "icona": random.choice(icone),
            "colore": fake.hex_color(),
            "codice_sotto_categoria": f"SOTTO_{suffisso}",
            "nome_sotto_categoria": f"Sotto categoris {suffisso}",
        }
    
    @staticmethod
    def genera_dati_nuova_spesa_operativa():
        oggi = date.today()
        scadenza = oggi + timedelta(days=random.randint(10, 60))
        faker = Faker("it_IT")
        return {
            "descrizione": f"Spesa Test {random.randint(1000, 9999)}",
            "imponibile": round(random.uniform(50, 2000), 2),
            "data": oggi.strftime("%Y-%m-%d"),
            "data_scadenza": scadenza.strftime("%Y-%m-%d"),
            "fornitore": "Fornitore Test SRL",
            "numero_fattura": f"FT-{random.randint(1000, 9999)}",
            "note": faker.paragraph(nb_sentences=2),
        }
    @staticmethod
    def genera_percorso_fattura_test():
        cartella_utils = os.path.dirname(os.path.abspath(__file__))  # utils/
        cartella_progetto = os.path.dirname(cartella_utils)          # root del progetto
        return os.path.join(cartella_progetto, "fixtures", "fatture", "fatturapa_esempio.xml")
